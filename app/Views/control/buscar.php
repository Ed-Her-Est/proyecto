    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8 form-container">
                <h2 class="form-title">Buscar Control</h2>
                <form action="<?= base_url('index.php/control/buscar/'); ?>" method="GET">
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                ...
                <!-- Tabla de controles con clases de Bootstrap -->
                <table class="table table-bordered table-striped text-center">
                    ...
                    <tbody>
                        <?php foreach ($controles as $control) : ?>
                            <tr>
                                <td><?= $control['fechaConsulta'] ?></td>
                                <td><?= $control['telefono'] ?></td>
                                <td><?= $control['Paciente_ID'] ?></td>
                                <td><?= $control['Consultorio_ID'] ?></td>
                                <td>
                                    <!-- Enlace para editar con botón de Bootstrap -->
                                    <a href="<?= base_url('control/editar/' . $control['ID_Control']) ?>" class="btn btn-primary btn-sm">Editar</a>

                                    <!-- Formulario para eliminar con botón de Bootstrap -->
                                    <form action="<?= base_url('control/eliminar/' . $control['ID_Control']) ?>" method="POST" style="display:inline;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de que deseas eliminar este control?')">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach ?>
