...
<head>
    ...
    <title>Lista de Datos Generales</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-12">
                <h2 class="text-center mb-4">Datos Generales</h2>
                ...
                <table class="table table-bordered table-striped text-center">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($datosGenerales as $dato) : ?>
                            <tr>
                            <td><?= ucfirst($dato['Paciente_ID']) ?></td>    
                            <td><?= ucfirst($dato['nombre']) ?></td>
                                <td><?= ucfirst($dato['domicilio']) ?></td>
                                <td><?= $dato['numeroEmergencia'] ?></td>
                                <td><?= $dato['edad'] ?></td>
                                <td><?= $dato['estatura'] ?></td>
                                <td><?= $dato['peso'] ?></td>
                                <td><?= ucfirst($dato['estadoCivil']) ?></td>
                                <td><?= ucfirst($dato['escolaridad']) ?></td>
                                <td><?= ucfirst($dato['religion']) ?></td>
                                <td><?= ucfirst($dato['motivoConsulta']) ?></td>
                                <td>
                                <div class="container">
                                    <!-- Enlace para editar -->
                                    <a href="<?= base_url('datosgenerales/editar/' . $dato['ID_DatosGenerales']) ?>" class="btn btn-primary">Editar</a>

                                    <!-- Formulario para eliminar -->
                                    <form action="<?= base_url('datosgenerales/delete/' . $dato['ID_DatosGenerales']) ?>" method="POST" style="display:inline;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="submit" class="btn btn-danger" onclick="return confirm('¿Estás seguro de que deseas eliminar estos datos generales?')">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
...
<div class="text-center mb-4">
    <a href="/signosvitales/agregar" class="btn btn-success">Agregar Signos Vitales</a>
</div>
        </div>
    </div>
